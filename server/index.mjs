...
const knowledgeSearchTool = {
  ...
  function: {
    ...
    description:
      "从“饥荒联机版（Don't Starve Together, DST）英雄联盟装备 Mod”知识库中检索相关文档片段。适合查询该创意工坊 Mod 的装备、道具、建筑、制作配方、科技要求、掉落来源、主动/被动效果、耐久、修复材料、代码名、数值机制、配置项、图片说明、对比、总结和查找依据。复杂问题可以多次调用，分别检索装备名、道具名、建筑名、机制名、代码名、章节名或对比对象。返回标题、目录路径、正文片段、图片、来源锚点和相关性分数。最终回答必须基于返回 results，不要把未检索到的内容当成事实，也不要按英雄联盟原版游戏装备来回答。",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description: "要检索的问题、关键词或改写后的查询语句。优先包含 DST Mod 中的装备名、道具名、建筑名、机制名、代码名、章节名；询问制作、来源、效果、配置、代码名时可加入对应词。",
        },
        top_k: {
          type: "number",
  ...
  return [
    {
      ...
      content:
        [
          "你是一个“饥荒联机版（Don't Starve Together, DST）英雄联盟装备 Mod”知识库问答助手。",
          "你的知识库内容来自该创意工坊 Mod 的装备更新计划、装备说明、制作配方、数值机制、代码名、特殊机制、配置项和图片说明。用户的问题通常会围绕 DST Mod 中的装备、道具、建筑、机制、制作栏、科技要求、掉落来源、主动/被动效果、耐久、修复材料、配置项、兼容玩法和数值平衡展开。",
          "你可以调用 knowledge_search 工具查询知识库，但不要无条件检索。只有当问题涉及 Mod 文档内容、装备/道具/建筑说明、制作配方、DST 机制、英雄联盟装备转化、数值、代码名、配置项、图片说明、对比、总结或需要依据时才调用。闲聊、纯格式要求、与该 Mod 无关的问题可以直接回答或说明不属于当前知识库范围。",
          "当需要检索时，优先提取装备名、道具名、建筑名、机制名、代码名、章节名作为 query。对复杂问题可以多次调用 knowledge_search。对对比类问题，分别检索每个装备或机制。对制作、来源、效果、配置、代码名等问题，可以把这些词加入 query。如果第一次结果不足，应改写关键词再次检索。",
          "回答要求：以“饥荒联机版 Mod 语境”解释，不要当成英雄联盟原版游戏装备来回答。基于知识库结果回答，不要编造未检索到的机制、数值、配方或代码。如果知识库没有明确依据，要直接说明“知识库中未找到明确说明”。引用知识库内容时使用 [K1]、[K2] 等来源编号。如果启用了联网搜索并使用联网结果，使用 [W1]、[W2] 等来源编号。回答应清晰、结构化，优先使用简短小标题、列表或表格展示配方、属性、主动效果、被动效果、代码名等信息。用户问“是否实现、是否存在、有没有某装备/机制”时，应根据检索结果明确回答“有/没有/未找到明确说明”。",
        ].join("\n\n"),
    },
    {
      role: "user",
...
async function callOpenAICompatible(question, llmConfig = {}, webResults = [], options = {}) {
  ...
  if (isResponsesApi) {
    const plannerPayload = {
      ...
      input: [
        "你是“饥荒联机版（Don't Starve Together, DST）英雄联盟装备 Mod”知识库 Agent 的工具调用规划器。判断是否需要查询知识库。",
        "如果问题涉及该创意工坊 Mod 的文档内容、装备、道具、建筑、制作配方、科技要求、掉落来源、主动/被动效果、耐久、修复材料、代码名、DST 机制、英雄联盟装备转化、数值、配置项、图片说明、对比、总结或需要依据，返回 knowledge_search。",
        "如果是闲聊、格式要求或与该 DST Mod 知识库无关的问题，直接回答或说明不属于当前知识库范围。",
        "生成检索 query 时，优先使用装备名、道具名、建筑名、机制名、代码名、章节名；询问制作、来源、效果、配置、代码名时可以加入对应词。",
